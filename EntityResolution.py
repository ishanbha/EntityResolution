...
def countTokens(vendorRDD):
    ...
    """
    count = vendorRDD.map(lambda (a,b):len(b)).reduce(lambda a,b:a+b)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    length = vendorRDD.count()
    li = vendorRDD.takeOrdered(length,lambda s: -1 * len(s[1]))
    return li
...
def idfs(corpus):
    ...
    tokenCountPairTuple = uniqueTokens.map(lambda s:(s,1.0))
...
def cosineSimilarity(string1, string2, idfsDictionary):
    ...
    w1 = tfidf(list1,idfsDictionary)
    w2 = tfidf(list2,idfsDictionary)
    return cossim(w1, w2)
...
def computeSimilarity(record):
    ...
    googleValue = tokenize(googleRec[1])
    amazonValue = tokenize(amazonRec[1])
    googleValue = ' '.join(googleValue)
    amazonValue = ' '.join(amazonValue)
...
avgSimDups = (trueDupsRDD.map(lambda s:s[1]).reduce(lambda a,b:a+b))/trueDupsCount

nonDupsRDD = (sims
              .subtractByKey(goldStandard))
nonDupsCount = nonDupsRDD.count()
avgSimNon = (nonDupsRDD.map(lambda s:s[1]).reduce(lambda a,b:a+b))/nonDupsCount
...
amazonWeightsBroadcast = sc.broadcast(amazonWeightsRDD.map(lambda s:(s[0],s[1])).collectAsMap())
googleWeightsBroadcast = sc.broadcast(googleWeightsRDD.map(lambda s:(s[0],s[1])).collectAsMap())
def fastCosineSimilarity(record):
    ...
    s = 0
    for tok in tokens:
        s += amazonWeightsBroadcast.value[amazonRec][tok]*googleWeightsBroadcast.value[googleRec][tok]
